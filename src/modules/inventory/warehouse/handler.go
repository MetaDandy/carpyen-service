...
func NewWarehouseHandler(service Service) Handler {
...
func (h *handler) RegisterRoutes(router fiber.Router) {
	...
	warehouse.Use(middleware.Jwt())

	warehouse.Post("/", middleware.RequireRole([]enum.Role{enum.RoleAdmin, enum.RoleChiefInstaller}), h.create)
	warehouse.Get("/", h.findAll)
	warehouse.Get("/:id", h.findByID)
	warehouse.Patch("/:id", middleware.RequireRole([]enum.Role{enum.RoleAdmin, enum.RoleChiefInstaller}), h.update)
	warehouse.Delete("/:id", middleware.RequireRole([]enum.Role{enum.RoleAdmin, enum.RoleChiefInstaller}), h.softDelete)
}
...
func (h *handler) findAll(c *fiber.Ctx) error {
	...
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, "Failed to retrieve warehouses")
	}
